...
export default {
  size(input) {
    return __guard__(input, x => x.length) != null
      ? __guard__(input, x => x.length)
      : 0;
  },
  downcase(input) {
  ...
  sort(input, property) {
    ...
    return toIterable(input)
      .map(item =>
        Promise
          .resolve(__guard__(item, x => x[property]))
          .then(key => ({ key, item })))
      .then(array => array
        .sort(function(a, b) {
          let left, left1;
  ...
  map(input, property) {
    ...
    return toIterable(input).map(e => __guard__(e, x => x[property]));
  },
  escape(input) {
  ...
  default(input, defaultValue) {
    ...
    let blank = (left = __guardMethod__(input, 'isBlank', o => o.isBlank())) !=
      null
      ? left
      : isBlank(input);
    if (blank) {
